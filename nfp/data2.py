...
compress_adj_matrix = False
MAX_NUMBER_ATOM = 140  # should be more than 132 for tox21 dataset.
filepath_train = 'data/tox21_train.pkl'
filepath_val = 'data/tox21_val.pkl'
filepath_etc = 'data/tox21_dataset.npz'
...
def _convert_dataset(dataset, atom2id):
    ...
    for d in dataset:
        ...
        ret.append((adj_array, atom_array, label))
    return ret
...
def construct_dataset():
    ...
    for data in [train, val]:
        ...
        for d in data:
            ...
            if compress_adj_matrix:
                for adj_elem in adj:
                    max_degree = max(max_degree, len(adj_elem))
                    min_degree = min(min_degree, len(adj_elem))
            else:
